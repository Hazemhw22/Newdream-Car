...
export default function LuxuryCars({ cars }: LuxuryCarsProps) {
  ...
  if (luxuryCars.length === 0) {
    return <div className="text-center text-gray-500 py-8">אין רכבי יוקרה כרגע.</div>;
  }
          ...
          <div key={car.id} className="relative">
            <CarCard
              car={{
                ...car,
                id: String(car.id),
                title: car.title ?? car.name ?? "רכב יוקרה", // fallback to car.name and then to default if title is undefined
                images: car.images ?? [], // ensure images is always a string[]
              }}
            />
            <Badge className="absolute top-3 right-3 bg-purple-700 text-white">יוקרה</Badge>
          </div>
        ))}
      </div>
    </section>
